...
public:
  ...
  void eval(Solution & x, int nMax) {
    ...
    eval(x, nMax);
    int best = x.fitness();
    int tmpBest = 0, nbIndices = x.indices.size();
    int randomIndice = 0, randomValue = 0;
    x.setFixedRules();
    //std::cout << "First: " << best << std::endl;
  
    for(int i = 0; i < maxIter; i++){
      randomIndice = rand() % nbIndices;
      randomValue = rand() % FIRE;

      tmpRule[x.indices.at(randomIndice)] = randomValue;
      eval(x, nMax);
      tmpBest = x.fitness();
    ...
    std::cout << "Final best : " << best << std::endl;
  }


	
	
	int hillClimberFirst(int maxEval){
