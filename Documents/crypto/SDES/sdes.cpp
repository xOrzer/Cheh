...
string F (string st, string cle){
    ...
    for(int i=stm.length()/2; i<(int)stm.length(); i++){
        st2+=stm[i];
    }
...
string sdesInverse(string st, string ci){
    string cle1 = K2(ci);    
    string cle2 = K1(ci);
    string st1 = "";
    string st2 = "";
...
char dechiffrement(char a, string cle){
	int x = a;
    char r;
    string st = base2(x);
    st = sdesInverse(st,cle);
    x = base10(st);
    r=x;
...
int main(){
	...
	string decrypt = "";
	string r = "";
	string cle = "1111011001";

	for(int i=0; i<text.length(); i++){
        r+=cryptageLettre(text[i],cle);
        decrypt+=dechiffrement(r[i],cle);
    }
	
	   cout<<"cryptage : " <<r<<endl;
	   cout<<"dechiffrement : " <<decrypt<<endl;
	
	/* Ebauche lecture fichier
	 * 
	 * string r="";
